...
public sealed class LocalBoxArtCache
{
    ...
    public LocalBoxArtCache(string? rootDir = null)
    {
        _root = string.IsNullOrWhiteSpace(rootDir)
            ? DefaultRoot()
            : rootDir.Trim();
    }
    ...
    public static string SanitizeSerialFolder(string serial)
    {
        if (string.IsNullOrWhiteSpace(serial))
            return "_unknown";
        string s = MediaVerify.NormalizeSerial(serial.Trim());
    ...
    public string? TryGet(string serial, BoxArtKind kind = BoxArtKind.Flat)
    {
        ...
        try
        {
            if (File.Exists(path))
            {
                ...
                if (fi.Length > 0) return path;
            }
        }
        catch
        {
            // ignore IO errors — treat as miss
        }
        return null;
    }
    ...
    /// <summary>
    /// Tiny valid 1×1 JPEG (gray) used as optional placeholder when online fetch fails.
    /// ~600 bytes; safe to write as box.jpg.
    /// </summary>
    public static byte[] MinimalPlaceholderJpeg()
    {
        // Minimal JFIF 1×1 grayscale JPEG (public domain / hand-built).
        return new byte[]
        {
