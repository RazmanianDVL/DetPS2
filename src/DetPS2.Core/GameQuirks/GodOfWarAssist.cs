...
public sealed class GodOfWarAssist : IGameQuirkModule
{
    ...
    /// <summary>Wave-8/9b: early TOC/PART1 FILEIO open before type-2 body.</summary>
    private bool _preType2FileIo;
    /// <summary>Wave-8/9b: LoadWad / stream-table bind seeded after host R_SHELL/TIT1.</summary>
    private bool _loadWadSeeded;
    /// <summary>Wave-9b: hang-escapes from real 0x27DBF0 stream-follow (no permanent soft-ok plant).</summary>
    private int _dbf0HangEscapes;
    private ulong _dbf0EnterCyc;
    /// <summary>Wave-9b: sticky — worker PC observed in 0x27DBF0 / follow band.</summary>
    private bool _dbf0Seen;
    /// <summary>Wave-9b: forced 0x27E234 once so 0x27DBF0 can run before cmd clear.</summary>
    private bool _type2EpiForced;
    private ulong _type2EpiForcedCyc;
    /// <summary>Wave-9b: post-type-2 cmd posts observed at *0x310384 (retail next-cmd telemetry).</summary>
    private int _postType2CmdPosts;
    ...
    /// <summary>Wave-10/11B: host-finish mid-pack byte-copy at 0x26C3A4 (size gate ≤513).</summary>
    private int _streamPackFinishes;
    private ulong _lastStreamPackCyc;
    /// <summary>Wave-11B: escapes from 0x17ED70 circular list-unlink residual (w10 final PC).</summary>
    private int _listUnlinkEscapes;
    /// <summary>Wave-11B: force-finish stuck GIF DMA tag builders at 0x13F5xx (END/QWC).</summary>
    ...
    public void Reset()
    {
        ...
        _dbf0EnterCyc = 0;
        _dbf0Seen = false;
        _type2EpiForced = false;
        _type2EpiForcedCyc = 0;
        _postType2CmdPosts = 0;
        _lastWorkerCmdSeen = 0;
        _postType2SemaWakes = 0;
        _streamPackFinishes = 0;
        _lastStreamPackCyc = 0;
        _listUnlinkEscapes = 0;
        _dmaTagFinishes = 0;
        _dmaTagStickyCyc = 0;
    ...
    public void Step(Ps2System sys)
    {
        ...
        if (c >= 35_000_000 && sys.Gs.PixelsWritten == 0 && dataPc)
        {
            ...
            {
                // Wave-8b: after type-2 prefer stream-poll 0x26C0EC — LastGood/ra often
                // lands in 0x292Cxx / 0x2A14xx UnknownOpcode storms (claim100 residual).
                uint preferred = _type2Completed && sys.Cdvd.SectorsRead > 142
                    ? 0x0026C0ECu
        ...
        // Wave-6/7/8b: type-2 fails when slot/stream gates return 0:
        //   0x282DD0 / 0x281568 → 0x81010013 (null handle)
        //   0x281548 (→0x282270) + *sp==0 → 0x8101006F
        // Disasm 0x27DF30 (wave-8b):
        //   0x281568 v0==2 → set +0x888=1 and take FULL stream body (0x27DED8/0x282208/
        //   0x281E30/0x27DCC8 fill + strcmp + 0x27DBF0 follow). v0!=2 with +0x888==0
        //   early-exits to 0x27E208 and skips all stream fill → FRAME_1 stays 0.
        // Wave-8b: return v0=2 from 0x281568 so the real stream path runs; soft-complete
        // later clears +0x888 for v0=0 success after fill has executed.
        if (!_type2GatesPlanted && c >= 37_000_000 && sys.Cdvd.SectorsRead > 0
            && sys.Gs.PixelsWritten == 0 && sys.Gif.Path3Transfers == 0)
        {
            // 0x282DD0: non-null handle gate — li v0,1; jr ra
            sys.Memory.Write32(0x00282DD0u, 0x24020001u);
            sys.Memory.Write32(0x00282DD4u, 0x03E00008u);
            sys.Memory.Write32(0x00282DD8u, 0x00000000u);
            // 0x281568: MUST return 2 (s4) to enter full stream body (wave-8b).
            sys.Memory.Write32(0x00281568u, 0x24020002u); // addiu v0, zero, 2
            sys.Memory.Write32(0x0028156Cu, 0x03E00008u); // jr ra
            sys.Memory.Write32(0x00281570u, 0x00000000u); // nop
            ...
            // 0x289B70 memcmp-like (stream name): return 0 (match) so body continues.
            ...
            // Wave-10B disasm: 0x27DBF0 loops jal 0x27D7C8 while cursor < *(slot+0x170).
            // Soft-ok the per-item worker so one iteration (or empty count) cannot hang:
            //   *a2=0 (status ok), v0=0, jr ra. (sw zero,0(a2) = 0xACC00000 — a2=reg6)
            sys.Memory.Write32(0x0027D7C8u, 0xACC00000u); // sw zero, 0(a2)  — status out
            sys.Memory.Write32(0x0027D7CCu, 0x0000102Du); // daddu v0, zero, zero
            sys.Memory.Write32(0x0027D7D0u, 0x03E00008u); // jr ra
            sys.Memory.Write32(0x0027D7D4u, 0x00000000u); // nop
            ...
            // Wave-9b/10B: do NOT permanent soft-ok 0x27DBF0 — that skips real stream-follow.
            // Real body is hang-guarded in TryEscapeStreamFollowHang after host R_SHELL lands.
            // Soft-ok empty-graph helpers so full body is not starved before assets land.
            ...
            // +0x88C is set by the strcmp-match path (sw 1); seed 0 so body owns it.
            // +0x890 cursor used by 0x27DBF0 stream follow-up — seed 0 (empty ready).
            // Wave-10B: +0x800 handle → host R_SHELL so 0x27D7C8 match-path can see Fedo.
            for (uint i = 0; i < 4; i++)
            {
                uint baseOff = 0x0032C9C8u + i * 2200u;
                sys.Memory.Write32(baseOff + 0x888u, 0u);
                sys.Memory.Write32(baseOff + 0x88Cu, 0u);
                sys.Memory.Write32(baseOff + 0x890u, 0u);
                sys.Memory.Write32(baseOff + 0x894u, 0u);
            ...
            // 0x27E220 to take the success branch 0x27E234 (real epilogue: 0x27DBF0 + status).
            ...
            // Wave-8b live: body hit 0x81010086 (digit parse a2<11 at 0x27E0B8) before stream
            // helpers — force continue into 0x27E0CC fill path.
            sys.Memory.Write32(0x0027E0B8u, 0x10000004u); // b +4 → 0x27E0CC
            sys.Memory.Write32(0x0027E0BCu, 0x00000000u); // nop
            ...
            // Pre-seed stream slot records BEFORE body runs (digits + buffer ptrs + count).
            // Live claim: post-complete seed is too late — body already returned 0x86.
            // Wave-10B disasm 0x27DBF0: loop bound is *(slot+0x170), NOT +0x38.
            // Seed +0x170=0 so follow takes early success leave (cursor 0 ≮ count 0) without
            // spinning on poison count; +0x38 still seeded for other fill helpers.
            for (uint slot = 0; slot < 4; slot++)
            {
                ...
                // count/size so fill loop and follow-up see non-empty work
                if (sys.Memory.Read32(s1 + 0x38u) == 0)
                    sys.Memory.Write32(s1 + 0x38u, 1u);
                if (sys.Memory.Read32(s1 + 0x30u) == 0)
                    sys.Memory.Write32(s1 + 0x30u, 0x1000u);
                if (sys.Memory.Read32(s1 + 0x34u) == 0)
                    sys.Memory.Write32(s1 + 0x34u, 0u);
                if (sys.Memory.Read32(s1 + 0x3Cu) == 0)
                    sys.Memory.Write32(s1 + 0x3Cu, 1u);
                // 0x27DBF0 loop bound (disasm lw v0, 0x170(s3)) — empty-ready → success leave.
                sys.Memory.Write32(s1 + 0x170u, 0u);
            ...
            // Clear stale type-2 error from poison wrong-tid attempts so body can rewrite.
            sys.Memory.Write32(0x00310388, 0u);
            _type2GatesPlanted = true;
            _type2GatesPlantCyc = c;
            _type2MidBodySeen = false;
            // Wave-9b: open TOC/PART1 + host R_SHELL/TIT1 *before* type-2 body so
            // real 0x27DBF0 / LoadWad see Fedo during the epi slice (not only post-complete).
            ...
            if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1")
                Console.Error.WriteLine(
                    $"[GOW] plant type-2 FULL-stream gates v0=2 (w10b: +0x170=0, 0x27D7C8 soft-ok, magic streamObj) cyc={c}");
        }

        // Wave-9b: real 0x27DBF0 stream-follow hang guard (no permanent soft-ok plant).
        // Mid-body hang after host shell lands → minimal success leave + GIF/VIF1 arm
        // (credits DMA handlers; Path3MaskedByVif held; no invented GIF packets).
        if (c >= 38_000_000 && pc is >= 0x0027DBF0 and <= 0x0027DCC4
            && sys.Gs.PixelsWritten == 0)
            TryEscapeStreamFollowHang(sys, pc, c);

        // Wave-11B: stream-work size gate at 0x26C26C: if s0>=513 → intentional hang
        // spin at 0x26C288 (beq self). Live claim100c final PC=0x26C29C with host size
        // 0xBAA95. Force s0=512 and take healthy path 0x26C2A4 (magic check + pack copy).
        if (c >= 40_000_000 && _type2Completed && sys.Gs.PixelsWritten == 0
            && pc is >= 0x0026C26C and <= 0x0026C2A0)
        {
            uint s0sz = (uint)sys.EE.GetGpr(16).Lo;
            if (s0sz >= 513u || pc is >= 0x0026C288 and <= 0x0026C2A0)
            {
                sys.EE.SetGpr(16, new EmotionEngine.Gpr128 { Lo = 512 }); // s0 < 513
                sys.EE.PC = 0x0026C2A4; // healthy path: magic 0x100 check + pack
                sys.EE.COP0_Status &= ~0x6u;
                if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1"
                    && (_streamPackFinishes + _dmaTagFinishes) < 48)
                    Console.Error.WriteLine(
                        $"[GOW] escape stream size-hang s0=0x{s0sz:X} -> 0x26C2A4 cyc={c}");
            }
        }

        // Wave-10: host-finish stream-work mid-pack byte-copy at 0x26C3A4 so producer
        // reaches budget/update + 0x26C478 (*obj++) and DMA tag builders can run.
        if (c >= 40_000_000 && _type2Completed && sys.Gs.PixelsWritten == 0
            && pc is >= 0x0026C398 and <= 0x0026C3B0)
            TryFinishStreamPackCopy(sys, pc, c);

        // Wave-11B: do NOT rehome 0x13F540..0x13F6A8 as thrash (W10), but when the tag
        // builder is sticky mid-align-pad / poison cursor (live claim PC=0x13F670 for
        // 60M cycles), force-finish END 0x70000000 so FRAME chain can leave.
        if (c >= 40_000_000 && _type2Completed && sys.Gs.PixelsWritten == 0
            && pc is >= 0x0013F540 and <= 0x0013F6A8)
            TryFinishDmaTagBuilder(sys, pc, c);

        // Wave-9b/10/11B: after type-2, observe *0x310384 posts + SignalSema-wake.
        // Never invent type-3/4 (w7 thrash). Retail posters live at 0x27C4xx..0x27C8xx.
        if (c >= 40_000_000 && _type2Completed && sys.Gs.PixelsWritten == 0)
            TryPostType2CmdWake(sys, pc, c);

        // Wave-11B: 0x17ED28..0x17ED80 list-unlink by key. Live w10 residual PC=0x17ED70
        // (bnel a0,a1 loop) when next links never hit sentinel a0 → main never reaches
        // worker-cmd posters at 0x27C4xx. Force empty-list epilogue.
        if (c >= 40_000_000 && _type2Completed && sys.Gs.PixelsWritten == 0
            && pc is >= 0x0017ED28 and <= 0x0017ED80)
            TryEscapeListUnlink(sys, pc, c);
        ...
        // Pathological heap free/align loop inside real alloc (0x13DC78 band — NEVER empty-exit
        // the whole allocator). Live w2: divu/mfhi at 0x13DEE8 with s0=0x310380 (address
        // mistaken for alignment) → remainder never 0 → forever. Wave-7 residual 0x13DEA0
        // free-list walk (bnel a2 chain) with poison a2. Return arena block via $ra.
        if (c >= 38_000_000 && pc is >= 0x0013DE80 and <= 0x0013DF20)
        {
            uint s0 = (uint)sys.EE.GetGpr(16).Lo;
            uint a2 = (uint)sys.EE.GetGpr(6).Lo;
            ...
            bool badList = IsBadCursor(a2) || (a2 & 0x1FFFFFFFu) >= (uint)SystemMemory.RDRAM_SIZE - 8u;
            // Wave-7: final PC parked at 0x13DEA0 mid free-walk after type-2.
            bool midFree = pc is >= 0x0013DE80 and <= 0x0013DEAC
                && (badList || _type2Completed || _worldKickPulses > 32);
            if (badAlign || spunOut || midFree)
            {
                uint ra = (uint)(sys.EE.GetGpr(31).Lo & 0x1FFFFFFFUL);
                uint resume = ra;
                ...
                if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1"
                    && (c % 5_000_000) < 50_000)
                    Console.Error.WriteLine(
                        $"[GOW] escape heap-align/free loop pc=0x{pc:X8} s0=0x{s0:X8} a2=0x{a2:X8} " +
        ...
        // Wave-7: residual also lands at 0x233A58 (pre-dispatch prologue) — widen band.
        if (c >= 38_000_000 && pc is >= 0x00233A50 and <= 0x00233B34)
        ...
        // Wave-9/10/11B: residual thrash after type-2 / LoadWad seed:
        //   - MMI / stack-PC / flip-lock / host-buffer-as-PC → post-FreezeCache.
        // Wave-10 CRITICAL: do NOT rehome 0x13F540..0x13F6A8 — retail GIF/VIF DMA tag
        // builder (QWC patch + END 0x70000000). Escaping it as "sleep-cmd" killed FRAME.
        // Do NOT rehome mid stream-work pack producer 0x26C150..0x26C470 (mid-pack 0x26C3A4).
        ...
        if (c >= 40_000_000 && _type2Completed && sys.Cdvd.SectorsRead > 400
            ...
            && (pc is >= 0x00289A00 and <= 0x00289C00
                ...
                || pc is >= 0x0028F000 and <= 0x0028F200  // soft-float thrash
                || pc is >= 0x00290000 and <= 0x00293000  // 0x2928xx / 0x2903xx
                ...
                // Host asset buffers planted as PC (TIT1/R_SHELL/TOC)
                || pc is >= 0x01D00000 and <= 0x01FFFFFF
                || pc is >= 0x001D0000 and <= 0x001DFFFF
                || pc is >= 0x00130000 and <= 0x00131000)
            // Throttle hard — claim100b fought permanent cache-wb stub every 100k.
            && (c % 500_000UL) < 50_000UL)
        {
            sys.Memory.Write32(0x002AC7D0, 1u);
            sys.Memory.Write32(0x002A1338, 0);
            RepairCurrentSpIfPoison(sys);
            // Only re-publish LoadWad table when not mid pack-producer (avoids resetting
            // *streamObj counter 0x100→ mid-work).
            if (_loadWadSeeded && pc is not (>= 0x0026C150 and <= 0x0026C470))
            ...
            {
                // Prefer post-FreezeCache so main can reach worker-cmd posters
                // (0x27C4xx type-3/5/6/7) and GIF DMA tag builders — not only stream-poll.
                // Alternate stream body every other escape so LoadWad consumers still run.
    ...
    /// <summary>
    /// Object method dispatch (disasm 0x233A50..0x233B40): loads flags from <c>a1</c>,
    ...
    /// Wave-7 residual also parks at 0x233A58. Snap to restore epilogue at 0x233B38.
    /// </summary>
    ...
    private static uint PickSafeResume(Ps2System sys, uint preferred)
    {
        ...
        static bool IsDeathBand(uint p) =>
            ...
            or (>= 0x0013DE80 and <= 0x0013DF20)  // heap free/align residual (wave-7 0x13DEA0)
    ...
    private bool TryYieldToPendingWorker(Ps2System sys, uint pc, ulong c)
    {
        ...
        // Wave-7: observe real type-2 progress on the worker thread.
        // Body is 0x27DF30..0x27E2C8 but also callees (cache-wb 0x294xxx, strcmp 0x289xxx,
        // stream helpers). Any non-idle/non-WaitSema worker PC while cmd==2 counts.
        if (currentIsWorker && !poisonFrame && cmdPending && cmdType == 2
            && !onIdle && pc is not (>= 0x00293C00 and <= 0x00293C90)
            && pc is >= 0x00100000 and < 0x00300000)
            _type2MidBodySeen = true;
        if (currentIsWorker && !poisonFrame
            && pc is >= 0x0027DF30 and <= 0x0027E2C8)
            _type2MidBodySeen = true;
        if (currentIsWorker && !poisonFrame && !onIdle
            && pc is >= 0x0027CC34 and <= 0x00282000)
            _type2MidBodySeen = true;

        // Wave-9b complete-once: do NOT complete on yields alone (w9b claim: mid=True
        // streamPast=False at 40M → 0x27DBF0 never entered; FRAME_1 stayed 0).
        // Require real streamPast / epi, or force epi then hang-guard 0x27DBF0, then clear.
        // Do not invent PATH3 packets / cmd types.
        ...
        bool type2Dbf0Band = currentIsWorker && pc is >= 0x0027DBF0 and <= 0x0027DCC4;
        if (type2Dbf0Band || type2Epilogue || type2StreamPast)
            _dbf0Seen = true;
        // Sticky from live EE PC after SwitchTo (incoming `pc` is often main 0x13F5).
        uint livePc = (uint)(sys.EE.PC & 0x1FFFFFFFUL);
        if (livePc is >= 0x0027D600 and <= 0x0027E2C8)
            _dbf0Seen = true;
        if ((worker.SavedPc & 0x1FFFFFFFUL) is >= 0x0027D600 and <= 0x0027E2C8)
            _dbf0Seen = true;
        bool type2Budget =
            _type2GatesPlanted
            && _type2GatesPlantCyc != 0
            && c >= _type2GatesPlantCyc + 1_200_000UL
            && (_type2MidBodySeen || _workerYields >= 4);
        // Hard ceiling: force success after long hang so host R_SHELL path can proceed.
        bool type2Ceiling =
            _type2GatesPlanted
            && _type2GatesPlantCyc != 0
            && c >= _type2GatesPlantCyc + 4_000_000UL
            && _workerYields >= 2;
        if (cmdPending && _type2GatesPlanted && !_type2Completed
            && _workerYields >= 2
            && (type2Epilogue || type2StreamPast || type2Dbf0Band || _dbf0Seen
                || type2Budget || type2Ceiling))
        {
            uint res = sys.Memory.Read32(0x00310388);
            bool isErr = res is >= 0x81010000u and <= 0x8101FFFFu;
            // Always clear +0x888 (full body sets 1) and force success result before clear.
            for (uint i = 0; i < 4; i++)
            {
                uint baseOff = 0x0032C9C8u + i * 2200u;
                sys.Memory.Write32(baseOff + 0x888u, 0u);
                sys.Memory.Write32(baseOff + 0x88Cu, 1u); // match-path ready
                sys.Memory.Write32(baseOff + 0x890u, 0u);
            }
            sys.Memory.Write32(0x00310388, 0u); // success result word
            try
            {
                uint wsp = (uint)(sys.EE.GetGpr(29).Lo & 0x1FFFFFFFUL);
                if (wsp >= 0x00100000u && wsp + 0x220u < (uint)SystemMemory.RDRAM_SIZE)
                    sys.Memory.Write32(wsp + 0x210u, 0u);
            }
            catch { /* ignore */ }
            uint statusPtr = sys.Memory.Read32(0x0031038C);
            uint spPhys = statusPtr & 0x1FFFFFFFu;
            if (statusPtr != 0 && spPhys >= 0x00100000u && spPhys + 16 < (uint)SystemMemory.RDRAM_SIZE
                && (spPhys & 3) == 0)
            {
                sys.Memory.Write32(spPhys + 0, 2u);  // type
                sys.Memory.Write32(spPhys + 4, 1u);  // ok flag
                sys.Memory.Write32(spPhys + 8, 0u);
            }

            // Wave-10B: force fill (0x27E0CC) first, then epi (0x27E234) → 0x27DBF0.
            // w9b jumped straight to epi and never latched streamPast; fill soft-helpers still
            // need the PC band so status/+0x888 clear graph can run before follow.
            // dbf0Seen alone is not enough — need hang-escape or ≥1.2M epi slice so the
            // follow body gets a real timeslice (claim: dbf0Seen@+100k cleared too early).
            bool naturalStream = type2Epilogue || type2StreamPast || type2Dbf0Band
                || _dbf0HangEscapes > 0;
            bool epiSliceDone = _type2EpiForced
                && _type2EpiForcedCyc != 0
                && (c >= _type2EpiForcedCyc + 1_200_000UL || _dbf0HangEscapes > 0);
            bool sawStream = naturalStream || (epiSliceDone && _dbf0Seen);
            // Re-seed loop bound each attempt (body may rewrite +0x170 to poison).
            for (uint slot = 0; slot < 4; slot++)
            {
                uint s1 = 0x002A3318u + slot * 388u;
                if (s1 + 0x174u < (uint)SystemMemory.RDRAM_SIZE)
                    sys.Memory.Write32(s1 + 0x170u, 0u);
            }
            if (!sawStream && !_type2EpiForced && !type2Ceiling)
            {
                // Pre-pin SavedPc BEFORE SwitchTo — forceDispatch would stomp to PostWait.
                const uint fillPc = 0x0027E0CC;
                ...
                if (!currentIsWorker)
                {
                    try
                    {
                        SwitchToWorkerThread(sys, k, worker, cmdType, pc, c, "type2-fill-epi",
                            forceDispatch: false, signalSema: true);
                    }
                    catch { /* ignore */ }
                }
                ...
                uint sp = 0x0031C500u;
                sys.EE.SetGpr(29, new EmotionEngine.Gpr128 { Lo = sp });
                sys.Memory.Write32(sp + 0x210u, 0u);
                // s1 = stream slot so 0x27DBF0 walk has a base (disasm uses s1).
                sys.EE.SetGpr(17, new EmotionEngine.Gpr128 { Lo = 0x002A3318u });
                sys.EE.SetGpr(16, new EmotionEngine.Gpr128 { Lo = 0x0032C9C8u + 0x88Cu });
                // a0=slot0, a1=status scratch for 0x27DBF0 when we land there.
                sys.EE.SetGpr(4, new EmotionEngine.Gpr128 { Lo = 0 });
                sys.EE.SetGpr(5, new EmotionEngine.Gpr128 { Lo = sp + 0x200u });
                sys.Memory.Write32(sp + 0x200u, 0u);
                // Re-pin after SwitchTo (restore may have used older SavedPc snapshot).
                worker.SavedPc = fillPc;
                worker.Sleeping = false;
                worker.WaitSemaId = 0;
                sys.EE.PC = fillPc;
                sys.EE.COP0_Status &= ~0x6u;
                _type2EpiForced = true;
                _type2EpiForcedCyc = c;
                if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1")
                    Console.Error.WriteLine(
                        $"[GOW] force type-2 fill 0x27E0CC then follow (w10b pin SavedPc) cyc={c}");
                // Do not clear cmd yet — let fill + real 0x27DBF0 run.
                return true;
            }
            // Still mid epi/fill slice — keep cmd pending and KEEP worker on-CPU so 0x27DBF0
            // can run (w9b claim: blocking sleep-cmd for 2M left main at 0x13F5 and
            // dbf0Esc=0). After ≥400k in fill, re-enter 0x27DBF0; hang-guard soft-leaves.
            if (!sawStream && _type2EpiForced && !epiSliceDone && !type2Ceiling)
            {
                uint wpcNow = (uint)(sys.EE.PC & 0x1FFFFFFFUL);
                bool fillSliceDone = _type2EpiForcedCyc != 0
                    && c >= _type2EpiForcedCyc + 400_000UL;
                bool inDbf0Now = wpcNow is >= 0x0027DBF0 and <= 0x0027DCC4;
                bool inFillNow = wpcNow is >= 0x0027E0CC and <= 0x0027E2C8;
                uint target = fillSliceDone || inDbf0Now
                    ? (inDbf0Now ? wpcNow : 0x0027DBF0u)
                    : (inFillNow ? wpcNow : 0x0027E0CCu);
                worker.SavedPc = target;
                worker.Sleeping = false;
                worker.WaitSemaId = 0;
                if (!currentIsWorker)
                {
                    try
                    {
                        SwitchToWorkerThread(sys, k, worker, cmdType, pc, c, "type2-epi-hold",
                            forceDispatch: false, signalSema: true);
                    }
                    catch { /* ignore */ }
                }
                // Always pin worker onto fill/follow — claim left toPc=WaitSema forever.
                RepairWorkerCalleeSaved(sys, worker);
                sys.EE.SetGpr(4, new EmotionEngine.Gpr128 { Lo = 0 }); // a0 = slot 0
                uint sp2 = (uint)(sys.EE.GetGpr(29).Lo & 0x1FFFFFFFUL);
                if (sp2 < 0x00100000u || sp2 + 0x220u >= (uint)SystemMemory.RDRAM_SIZE)
                {
                    sp2 = 0x0031C500u;
                    sys.EE.SetGpr(29, new EmotionEngine.Gpr128 { Lo = sp2 });
                }
                sys.EE.SetGpr(5, new EmotionEngine.Gpr128 { Lo = sp2 + 0x200u }); // a1 status
                sys.Memory.Write32(sp2 + 0x200u, 0u);
                sys.EE.SetGpr(17, new EmotionEngine.Gpr128 { Lo = 0x002A3318u });
                worker.SavedPc = target;
                worker.Sleeping = false;
                worker.WaitSemaId = 0;
                sys.EE.PC = target;
                sys.EE.COP0_Status &= ~0x6u;
                if (target is >= 0x0027D600 and <= 0x0027E2C8)
                    _dbf0Seen = true;
                return true;
            }
            ...
            cmdPending = false;
            _armedWorkerCmd = 0;
            _type2Completed = true;
            _type2CompletedCyc = c;
            _lastWorkerCmdSeen = 0;
            // Park worker at idle; rehome any thread off worker .text so main can post
            // next cmds / LoadWad (live after clear → poison SP word-scan).
            if (currentIsWorker || onWorkerText)
            {
                if (currentIsWorker)
                {
                    sys.EE.PC = WorkerIdleLo;
                    sys.EE.COP0_Status &= ~0x6u;
                }
                else
            ...
            if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1")
                Console.Error.WriteLine(
                    $"[GOW] complete worker cmd={cmdType} resWas=0x{res:X8} " +
                    $"mid={_type2MidBodySeen} epi={type2Epilogue} streamPast={type2StreamPast} " +
                    $"dbf0={type2Dbf0Band} dbf0Seen={_dbf0Seen} dbf0Esc={_dbf0HangEscapes} " +
                    $"softOk={isErr} success=True cyc={c}");
        }
        ...
        // Wave-9b: during epi slice, SwitchTo worker but NEVER force PostWait (protect
        // 0x27E234/0x27DBF0 SavedPc). Outside slice, normal sleep-cmd dispatch.
        if (cmdPending && worker.Sleeping && worker.WaitSemaId is >= 1 and <= 256)
        {
            if (c - _lastWorkerYieldCyc < 200_000UL) return false;
            bool inEpiSlice = _type2EpiForced && _type2EpiForcedCyc != 0
                && (c - _type2EpiForcedCyc) < 2_000_000UL && !_type2Completed;
            _armedWorkerCmd = cmdType;
            return SwitchToWorkerThread(sys, k, worker, cmdType, pc, c,
                inEpiSlice ? "sleep-cmd-epi" : "sleep-cmd",
                forceDispatch: inEpiSlice ? false : (atGate || !midBody));
        }
    ...
    private void MaybeKickWorldProgress(Ps2System sys, uint pc, ulong c)
    {
        ...
        // Post-ready body / work path. Wave-10B: keep magic-0x100 streamObj for 0x26C2C4.
        // Wave-10/11B: never Refresh mid pack-producer 0x26C150..0x26C470 — *obj is a counter
        // starting at magic 0x100 and incremented by 0x26C478; resetting restarts the graph.
        else if (pc is >= 0x0026C0EC and <= 0x0026C600 && sys.Gs.PixelsWritten == 0
                 && sys.Cdvd.SectorsRead > 0)
        {
            TryArmPendingStreamJob(sys, c);
            sys.Memory.Write32(0x002A1338, 0);
            bool midPack = pc is >= 0x0026C150 and <= 0x0026C470;
            uint tab0 = sys.Memory.Read32(0x002A1358);
            uint tPhys = tab0 & 0x1FFFFFFFu;
            bool seedOk = _loadWadSeeded
                && tab0 != 0
                && tPhys >= 0x00100000u
                && tPhys + 0x10u < (uint)SystemMemory.RDRAM_SIZE
                && (tPhys & 3) == 0
                && sys.Memory.Read32(tPhys) is >= 0x00000100u and <= 0x00000200u // magic / counter
                && sys.Memory.Read32(tPhys + 4) == 0x01E00000u; // payload → R_SHELL
            if (_loadWadSeeded && !seedOk && !midPack)
            {
                // Refresh rather than zero — zero kills the only shell consumer path.
                RefreshLoadWadStreamTable(sys);
                seedOk = true;
            }
            else if (!seedOk && !midPack)
            {
                sys.Memory.Write32(0x002A1358, 0);
                sys.Memory.Write32(0x002A1378, 0);
            }
            else if (midPack)
                seedOk = _loadWadSeeded; // leave *obj counter alone mid-pack
            if (seedOk && pc is >= 0x0026C0EC and <= 0x0026C600)
            {
                // Re-arm s0/s1 = streamObj only when not mid-pack (would clobber cursor GPRs).
                if (!midPack)
                {
                    sys.EE.SetGpr(16, new EmotionEngine.Gpr128 { Lo = 0x01CFE000UL });
                    sys.EE.SetGpr(17, new EmotionEngine.Gpr128 { Lo = 0x01CFE000UL });
                }
                sys.Memory.Write32(0x002AC7D0, 1u); // flip/kick enable
            }
            if (pc is >= 0x0026C4B0 and <= 0x0026C5F0 && !seedOk)
            {
        ...
        if (pc is >= 0x001312C0 and <= 0x001312E8)
            TryEscapeLinkSearch(sys, pc, c);
        if (pc is >= 0x00233A50 and <= 0x00233B34)
            TryEscapeObjectDispatch(sys, pc, c);
        ...
        // Wave-11B: 0x17ED70 circular list-unlink residual (w10 claim final PC).
        if (pc is >= 0x0017ED28 and <= 0x0017ED80 && sys.Gs.PixelsWritten == 0
            && _type2Completed)
            TryEscapeListUnlink(sys, pc, c);
    ...
    /// <summary>
    /// Wave-7/8/9b: after type-2 complete, host-load GODOFWAR.TOC + shell WAD into EE scratch and
    /// credit honest CD sectors so telemetry leaves IRX-only 142. Also re-arm 989snd done-magic,
    /// keep type-2 table +0x888 clear, seed LoadWad bind, and leave empty-SIF / freelist thrash
    /// for stream poll. Does not invent PATH3 GIF packets (Path3MaskedByVif held).
    /// </summary>
    private void TryPostType2StreamKick(Ps2System sys, uint pc, ulong c)
    {
        ...
        if (_type2PostStreamKicks == 1)
        {
            ...
            try
            {
                ...
                if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1")
                    Console.Error.WriteLine(
                        $"[GOW] FILEIO force-open TOC fd={fdToc} PART1 fd={fdPak} cyc={c}");
            }
            catch { /* ignore */ }
        }
        ...
        else if (_type2PostStreamKicks == 4)
        {
            ...
            try
            {
                // PART1.PAK LBA 1547 → disc byte = 1547*2048; member off is within PAK.
                const uint PakBase = 1547u * 2048u;
                TryOpenPakMemberFd(sys, "R_SHELL.WAD", PakBase);
                TryOpenPakMemberFd(sys, "TIT1E1_2.VPK", PakBase);
            }
            catch { /* ignore */ }
        }
        ...
        TryArmPendingStreamJob(sys, c);
        // Flip/kick enable so display path can arm FRAME after shell decode (disasm 0x140A00).
        if (_loadWadSeeded || _type2PostStreamKicks >= 2)
            sys.Memory.Write32(0x002AC7D0, 1u);

        // Wave-8b/10B: seed type-2 stream slot records so post-success main/worker can see
        // host-extracted R_SHELL / TIT1 buffers (natural consumers; no GIF invent).
        // Slot stride 388 @ 0x2A3318. +0x170 is 0x27DBF0 loop bound (disasm).
        if (_type2PostStreamKicks is >= 2 and <= 8)
        {
            for (uint slot = 0; slot < 2; slot++)
            {
                ...
                sys.Memory.Write32(s1 + 0x170u, 0u); // follow loop bound empty-ready
                if (sys.Memory.Read32(s1 + 0x40u) == 0)
                    sys.Memory.Write32(s1 + 0x40u, buf);
                if (sys.Memory.Read32(s1 + 0x44u) == 0)
                    sys.Memory.Write32(s1 + 0x44u, buf);
        ...
        // Keep main + worker Started after type-2 (live w8b: all Started=false → dead EE).
        // Rejected: force-post worker type-3/4 (claim100e: 3.3M WaitSema thrash, no PATH3).
        try
        {
            var kk = sys.Hle?.Kernel;
            if (kk != null)
            {
                foreach (var t in kk.AllThreads)
                {
                    ...
                    if (!t.Started && t.Entry is >= 0x00100000 and < 0x00300000)
                    {
                        ...
                        kk.WakeupThread(t.Id);
                }
            }
        }
        catch { /* ignore */ }

        // Credit GIF/VIF1 so any natural PATH3 that type-2 armed can drain (no ungating).
        try
        {
            sys.Intc.SetMask(sys.Intc.Mask | (1u << (int)Intc.InterruptSource.DmaController)
                | (1u << (int)Intc.InterruptSource.VBlankStart)
    ...
    /// <summary>
    /// Wave-8/9b: open GODOFWAR.TOC + PART1.PAK before type-2 body so LoadWad/FILEIO bind
    /// can see the pack while the FULL-stream path runs.
    /// </summary>
    ...
    /// <summary>
    /// Wave-8/9b: after host R_SHELL/TIT1 land, plant stream-table pointers the type-2
    /// follow-up and LoadWad path observe. Does not invent GIF packets — only EE state
    /// so natural decode can issue FRAME+PRIM (Path3MaskedByVif held).
    /// </summary>
    private void TrySeedLoadWadBind(Ps2System sys, ulong c)
    {
        ...
        for (uint i = 0; i < 4; i++)
        {
            ...
            if (tit0 != 0)
            {
                ...
                sys.Memory.Write32(baseOff + 0x80Cu, 0x00043E8Au); // first TOC size TIT1
            }
        }

        // Plant R_Shell name near scratch so strcmp/LoadWad path can see it.
        ...
        if (ctx == 0 || (ctx & 0x1FFFFFFFu) >= (uint)SystemMemory.RDRAM_SIZE)
            sys.Memory.Write32(0x002ACAC8, 0x01F00000u); // point at host TOC

        sys.Memory.Write32(0x002A1338, 0);
        sys.Memory.Write32(0x0029C7D0, 0);
        // Flip/kick enable (0x140A00 stores 1 here before Flip And Kick Lock).
        sys.Memory.Write32(0x002AC7D0, 1u);

        // Wave-11B poster gate (disasm 0x27C4D4): lw v1, *0x2A3310; beq v1,0 → 0x81018001.
        // Must be non-zero so retail posters at 0x27C4xx can arm. Stream slots start @0x2A3318.
        if (sys.Memory.Read32(0x002A3310u) == 0)
            sys.Memory.Write32(0x002A3310u, 0x002A3318u);

        // Wave-10B disasm 0x26C2BC: *obj must be 0x100 magic (beql v0,a0 → healthy path).
        ...
        if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1")
            Console.Error.WriteLine(
                ...
                $"tit0=0x{tit0:X8} streamObj=0x01CFE000 magic=0x100 cyc={c}");
    }

    /// <summary>
    /// Wave-10B: keep 989snd/stream slot table healthy. Disasm 0x26C2BC:
    ///   table[index] → obj; *obj must be 0x100 magic or path uses poison 0x2A1360 table.
    ///   Payload at +4, size at +8. Budget companion at 0x2A1360 must be ≥ work size.
    /// </summary>
    ...
    /// <summary>
    /// Wave-9b: hang-guard for real 0x27DBF0 stream-follow (no permanent soft-ok plant).
    /// Soft-ok at plant time skipped DMA that arms GIF → FRAME_1 stays 0 (w8/w8b residual).
    /// Allow real body a slice; if stuck mid-band after host shell, leave with *a1=0/v0=0
    /// and credit GIF/VIF1 handlers so any already-queued Path2/3 can drain.
    /// Path3MaskedByVif held; no invented GIF packets / cmd types.
    /// </summary>
    private void TryEscapeStreamFollowHang(Ps2System sys, uint pc, ulong c)
    {
        if (_dbf0HangEscapes >= 64) return;
        if (_dbf0EnterCyc == 0)
            _dbf0EnterCyc = c;
        // Give real body ≥400k cycles once, or immediate if shell already host-loaded.
        uint fedo = 0;
        try { fedo = sys.Memory.Read32(0x01E00000u); } catch { /* ignore */ }
        bool shellOk = fedo == 0x4665646Fu;
        bool budget = shellOk
            ? (c - _dbf0EnterCyc) >= 200_000UL || _dbf0HangEscapes > 0
            : (c - _dbf0EnterCyc) >= 800_000UL;
        if (!budget && _dbf0HangEscapes == 0) return;

        // Cursor / a1 often poison mid-hang — force status ok and leave via $ra.
        uint a1 = (uint)sys.EE.GetGpr(5).Lo;
        uint a1Phys = a1 & 0x1FFFFFFFu;
        if (a1 != 0 && a1Phys >= 0x00100000u && a1Phys + 4u < (uint)SystemMemory.RDRAM_SIZE
            && (a1Phys & 3) == 0)
            sys.Memory.Write32(a1Phys, 0u);

        // Keep type-2 table success-ready + stream-follow loop bound empty-ready.
        for (uint i = 0; i < 4; i++)
        {
            uint baseOff = 0x0032C9C8u + i * 2200u;
            sys.Memory.Write32(baseOff + 0x888u, 0u);
            sys.Memory.Write32(baseOff + 0x88Cu, 1u);
            sys.Memory.Write32(baseOff + 0x890u, 0u); // cursor
            uint s1 = 0x002A3318u + i * 388u;
            if (s1 + 0x174u < (uint)SystemMemory.RDRAM_SIZE)
                sys.Memory.Write32(s1 + 0x170u, 0u); // loop bound → early success leave
        }
        sys.Memory.Write32(0x00310388, 0u);
        if (_loadWadSeeded)
            RefreshLoadWadStreamTable(sys);
        ...
        uint ra = (uint)(sys.EE.GetGpr(31).Lo & 0x1FFFFFFFUL);
        uint resume = ra;
        if (!sys.Memory.IsLikelyEeCode(resume) || resume is < 0x00100000 or >= 0x00300000
            || resume is (>= 0x0027DBF0 and <= 0x0027DCC4))
            resume = 0x0027E234; // type-2 success epilogue band
        // Prefer natural re-entry of 0x27DBF0 prologue once so empty count can succeed
        // (hang at mid-loop 0x27DC6x with poison count → leave; next call with +0x170=0 exits).
        if (pc is >= 0x0027DC50 and <= 0x0027DC98 && _dbf0HangEscapes == 0)
            resume = 0x0027DBF0;
        sys.EE.SetGpr(2, new EmotionEngine.Gpr128 { Lo = 0 }); // v0 = ok
        // a0=slot0, a1=status for clean re-entry
        sys.EE.SetGpr(4, new EmotionEngine.Gpr128 { Lo = 0 });
        uint sp = (uint)(sys.EE.GetGpr(29).Lo & 0x1FFFFFFFUL);
        if (sp >= 0x00100000u && sp + 0x220u < (uint)SystemMemory.RDRAM_SIZE)
        {
            sys.Memory.Write32(sp + 0x200u, 0u);
            sys.EE.SetGpr(5, new EmotionEngine.Gpr128 { Lo = sp + 0x200u });
        }
        ...
        sys.EE.COP0_Status &= ~0x6u;
        _dbf0HangEscapes++;
        _dbf0EnterCyc = c; // re-arm for re-entry
        _dbf0Seen = true;

        // Minimal GIF arm: credit handlers so any Path2 traffic can complete drains.
        // Does not invent PRIM/FRAME packets — only IRQ/handler progress.
        try
        {
            sys.Intc.SetMask(sys.Intc.Mask | (1u << (int)Intc.InterruptSource.DmaController)
                | (1u << (int)Intc.InterruptSource.VBlankStart)
            ...
            sys.Dmac.EnableChannelIrq((int)Dmac.Channel.GIF);
            sys.Dmac.EnableChannelIrq((int)Dmac.Channel.VIF1);
            sys.Dmac.CreditOwedHandlerCall((int)Dmac.Channel.GIF, 8);
            sys.Dmac.CreditOwedHandlerCall((int)Dmac.Channel.VIF1, 8);
        }
        catch { /* ignore */ }
        ...
        if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1"
            && _dbf0HangEscapes <= 16)
            Console.Error.WriteLine(
                $"[GOW] 0x27DBF0 hang-escape n={_dbf0HangEscapes} shellOk={shellOk} " +
                $"pc=0x{pc:X8} -> 0x{resume:X8} gifP2={sys.Gif?.Path2Transfers ?? 0} cyc={c}");
    }

    /// <summary>
    /// Wave-11B: finish stuck GIF/VIF DMA tag builders at 0x13F540..0x13F6A8.
    /// W10 forbids rehoming these as thrash (they write QWC + END 0x70000000). Live claim
    /// parks mid-align-pad at 0x13F5F8/0x13F670 for tens of M cycles with poison cursor
    /// at *0x32F168, so FRAME chain never finalizes and main never reaches 0x27C4xx posters.
    /// Force-align cursor, write END tag, advance *0x32F168, return via $ra / 0x185FAC.
    /// Path3MaskedByVif held; no invented GIF packets / cmd types.
    /// </summary>
    private void TryFinishDmaTagBuilder(Ps2System sys, uint pc, ulong c)
    {
        ...
        uint cursor = sys.Memory.Read32(CursorPtr);
        ...
        // Sticky: mid-align-pad / END body for ≥200k cycles post type-2.
        ...
        // Return via $ra when real; else post-FreezeCache so main can reach posters.
        uint ra = (uint)(sys.EE.GetGpr(31).Lo & 0x1FFFFFFFUL);
        uint resume = ra;
        ...
        if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1"
            && _dmaTagFinishes <= 24)
            Console.Error.WriteLine(
                $"[GOW] finish DMA tag builder n={_dmaTagFinishes} pc=0x{pc:X8} " +
    ...
    /// Wave-10: host-finish stream-work mid-pack byte-copy at 0x26C3A4 so producer
    /// reaches budget/update + 0x26C478 (*obj++) and DMA tag builders can run.
    /// Size gate is s0&lt;513; refuse huge poison lengths.
    /// </summary>
    private void TryFinishStreamPackCopy(Ps2System sys, uint pc, ulong c)
    {
        if (_streamPackFinishes >= 128) return;
        if (_streamPackFinishes > 0 && c - _lastStreamPackCyc < 50_000UL) return;

        uint a1 = (uint)sys.EE.GetGpr(5).Lo;   // cursor
        uint s1 = (uint)sys.EE.GetGpr(17).Lo;  // length
        uint s2 = (uint)sys.EE.GetGpr(18).Lo;  // src
        uint a3 = (uint)sys.EE.GetGpr(7).Lo;   // dst
        if (s1 == 0 || a1 >= s1) return;
        uint remain = s1 - a1;
        if (remain > 0x800u) return; // size gate is 513; refuse huge poison lengths
        uint srcP = (s2 + a1) & 0x1FFFFFFFu;
        uint dstP = (a3 + a1) & 0x1FFFFFFFu;
        if (srcP < 0x00100000u || dstP < 0x00100000u) return;
        if (srcP + remain >= (uint)SystemMemory.RDRAM_SIZE) return;
        if (dstP + remain >= (uint)SystemMemory.RDRAM_SIZE) return;

        for (uint i = 0; i < remain; i++)
            sys.Memory.Write8(dstP + i, sys.Memory.Read8(srcP + i));

        sys.EE.SetGpr(5, new EmotionEngine.Gpr128 { Lo = s1 }); // a1 = s1 (done)
        sys.EE.SetGpr(2, new EmotionEngine.Gpr128 { Lo = 0 });
        sys.EE.PC = 0x0026C3F0; // post-copy: load index + budget update
        sys.EE.COP0_Status &= ~0x6u;
        _streamPackFinishes++;
        _lastStreamPackCyc = c;
        if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1"
            && _streamPackFinishes <= 24)
            Console.Error.WriteLine(
                $"[GOW] stream-pack host-finish n={_streamPackFinishes} remain={remain} " +
                $"src=0x{srcP:X8} dst=0x{dstP:X8} pc=0x{pc:X8} -> 0x26C3F0 cyc={c}");
    }

    /// <summary>
    /// Wave-11B: list-unlink-by-key at 0x17ED28 (disasm):
    ///   a0 = sentinel/head; a1 walks *a1; stop when a1==a0; match key at +4 vs v1.
    /// Live residual (w10 claim100): PC=0x17ED70 <c>bnel a0,a1,loop</c> with corrupt next
    /// links that never hit sentinel — freezes main so 0x27C4xx posters never run.
    /// Force empty-list epilogue (0x17ED78) when cursor is poison/OOB/self-ring.
    /// </summary>
    private void TryEscapeListUnlink(Ps2System sys, uint pc, ulong c)
    {
        if (_listUnlinkEscapes >= 256) return;
        // Already in epilogue — leave alone.
        if (pc is >= 0x0017ED78 and <= 0x0017ED80)
            return;

        uint a0 = (uint)sys.EE.GetGpr(4).Lo; // sentinel
        uint a1 = (uint)sys.EE.GetGpr(5).Lo; // cursor
        uint a0p = a0 & 0x1FFFFFFFu;
        uint a1p = a1 & 0x1FFFFFFFu;
        bool a0Bad = a0 == 0 || a0p < 0x00100000u || a0p + 8u >= (uint)SystemMemory.RDRAM_SIZE
            || (a0p & 3) != 0;
        bool a1Bad = a1 == 0 || a1p < 0x00100000u || a1p + 8u >= (uint)SystemMemory.RDRAM_SIZE
            || (a1p & 3) != 0;
        bool selfRing = !a1Bad && sys.Memory.Read32(a1p) == a1;
        // Mid-loop body (0x17ED48..0x17ED74) — force after first hit or always on poison.
        bool midLoop = pc is >= 0x0017ED48 and <= 0x0017ED74;
        if (!a0Bad && !a1Bad && !selfRing && !midLoop && _listUnlinkEscapes == 0)
            return;
        // Healthy short scan: if a1 already equals a0, natural empty path — just land epi.
        if (!a0Bad && a1 == a0 && !midLoop)
            return;

        // Empty-list: make *a0 = a0 so natural re-entry also exits, then snap to epi.
        if (!a0Bad)
            sys.Memory.Write32(a0p, a0);
        sys.EE.SetGpr(5, new EmotionEngine.Gpr128 { Lo = a0 }); // a1 = sentinel
        sys.EE.SetGpr(2, new EmotionEngine.Gpr128 { Lo = 0 });
        sys.EE.PC = 0x0017ED78; // ld ra; jr ra
        sys.EE.COP0_Status &= ~0x6u;
        _listUnlinkEscapes++;
        if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1"
            && _listUnlinkEscapes <= 24)
            Console.Error.WriteLine(
                $"[GOW] escape list-unlink 0x17EDxx pc=0x{pc:X8} a0=0x{a0:X8} a1=0x{a1:X8} " +
                $"-> 0x17ED78 n={_listUnlinkEscapes} cyc={c}");
    }

    /// <summary>
    /// Wave-9b/10/11B: after type-2 clear, retail posts next worker cmds at *0x310384 from
    /// poster helpers 0x27C4xx..0x27C8xx (type 3/5/6/7) once the queue word is 0.
    /// Trace posts; SignalSema-wake sleeping game threads so the poster can run.
    /// Never invent type-3/4 (w7 thrash). Path3MaskedByVif held.
    /// </summary>
    private void TryPostType2CmdWake(Ps2System sys, uint pc, ulong c)
    {
        if (_type2CompletedCyc == 0 || c < _type2CompletedCyc + 100_000UL) return;
        // Throttle.
        if ((c % 250_000UL) >= 50_000UL && _postType2SemaWakes > 0) return;

        uint cmd = sys.Memory.Read32(WorkerCmdTypePtr);
        if (cmd != _lastWorkerCmdSeen)
        {
            if (_lastWorkerCmdSeen != 0 || cmd != 0)
            {
                _postType2CmdPosts++;
                if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1"
                    && _postType2CmdPosts <= 24)
                {
                    var kk = sys.Hle?.Kernel;
                    int tid = kk?.CurrentThreadId ?? -1;
                    Console.Error.WriteLine(
                        $"[GOW] *0x310384 cmd 0x{_lastWorkerCmdSeen:X}→0x{cmd:X} " +
                        $"tid={tid} pc=0x{pc:X8} posts={_postType2CmdPosts} cyc={c}");
                }
            }
            _lastWorkerCmdSeen = cmd;
        }

        // Ensure queue word is 0 (not 0xFFFFFFFF) so retail beql v0,r0 posters can arm.
        // Disasm 0x27C524: lw v0,4(0x310380); beql v0,0 → post type; else 0x81010010.
        if (cmd == 0xFFFFFFFFu)
        {
            sys.Memory.Write32(WorkerCmdTypePtr, 0);
            cmd = 0;
        ...
        // Poster gate *0x2A3310 must be non-zero (disasm 0x27C4D4 / 0x27C5AC).
        if (sys.Memory.Read32(0x002A3310u) == 0)
            sys.Memory.Write32(0x002A3310u, 0x002A3318u);

        // Keep LoadWad table healthy (989snd may have stomped companions earlier).
        if (_loadWadSeeded && (c % 1_000_000UL) < 50_000UL)
        {
            uint so = sys.Memory.Read32(0x01CFE000u);
            if (so == 0xFFFFFFFFu || so == 0 || so > 0x200u)
                RefreshLoadWadStreamTable(sys);
        }

        // SignalSema / Wakeup sleeping threads so main can post next cmd / decode shell.
        // Cap wakes; never rewrite WorkerCmdTypePtr with invented cmd types.
        if (_postType2SemaWakes >= 128) return;
        try
        {
            var k = sys.Hle?.Kernel;
            if (k == null) return;
            ...
            foreach (var t in k.AllThreads)
            {
                ...
                // Worker with real pending cmd is handled by TryYieldToPendingWorker.
                if (t.Entry == WorkerThreadEntry && cmd is >= 2 and <= 100) continue;
                if (t.Sleeping && t.WaitSemaId is >= 1 and <= 256)
                {
                    try
                    {
                ...
                {
                    try { k.WakeupThread(t.Id); woke++; } catch { /* ignore */ }
                }
            }
            // Revive ExitThread'd game threads. Main often has Entry=0 after soft-exit —
            // plant post-FreezeCache continue so posters at 0x27C4xx can be reached.
            foreach (var t in k.AllThreads)
            {
                if (!t.Alive || t.Started) continue;
                if (t.Entry == WorkerThreadEntry) continue;
                uint entry = t.Entry;
                if (entry is < 0x00100000 or >= 0x00300000)
                {
                    // Main/boot thread with empty Entry — re-seed to post-FreezeCache.
                    if (t.Id == 1 || entry == 0)
                    {
                        try
                        {
                            // Kernel may reject Entry=0 Start — soft-land current EE instead.
                            t.Sleeping = false;
                            t.WaitSemaId = 0;
                            t.Started = true;
                            t.SavedPc = 0x00185FAC;
                            if (t.Id == k.CurrentThreadId || k.CurrentThreadId == t.Id)
                            {
                                sys.EE.PC = 0x00185FAC;
                                sys.EE.SetGpr(2, new EmotionEngine.Gpr128 { Lo = 0x00330000UL });
                                sys.EE.COP0_Status &= ~0x6u;
                ...
                try
                {
                    t.Sleeping = false;
                    t.WaitSemaId = 0;
                    k.StartAndMaybeSwitch(sys.EE, t.Id, switchNow: false, arg: 0,
                        fromSyscall: false);
                    woke++;
                }
                catch { /* ignore */ }
            ...
            if (woke > 0)
            {
                _postType2SemaWakes++;
                if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1"
                    && _postType2SemaWakes <= 16)
                    Console.Error.WriteLine(
                        $"[GOW] post-type2 sema-wake n={_postType2SemaWakes} woke={woke} " +
                        $"cmd=0x{cmd:X} pc=0x{pc:X8} cyc={c}");
            }
        }
        catch { /* ignore */ }

        // Keep flip/kick + stream ready after assets land.
        if (_loadWadSeeded || sys.Cdvd.SectorsRead > 400)
        {
            sys.Memory.Write32(0x002AC7D0, 1u);
            // Re-publish shell handles if something cleared them.
            uint fedo = sys.Memory.Read32(0x01E00000u);
            if (fedo == 0x4665646Fu)
            {
                for (uint i = 0; i < 2; i++)
    ...
    /// <summary>
    /// Read a disc file via <see cref="IopModuleHost.ReadDiscFileBytes"/> into EE RDRAM and
    /// credit <see cref="Cdvd.NoteHostReadSectors"/> (honest sector growth past IRX-only).
    /// </summary>
    private static bool TryHostLoadDiscFile(Ps2System sys, string name, uint dest, uint maxBytes)
    {
        try
        {
            var mods = sys.IopModules;
            if (mods == null) return false;
            byte[]? data = mods.ReadDiscFileBytes(name, (int)maxBytes)
                           ?? mods.ReadDiscFileBytes("cdrom0:\\" + name, (int)maxBytes)
                           ?? mods.ReadDiscFileBytes("cdrom0:" + name, (int)maxBytes);
            if (data == null || data.Length == 0) return false;
            return WriteHostBytes(sys, dest, data, maxBytes);
        }
        catch
        {
            return false;
    ...
    /// <summary>
    /// Extract a named member from PART1.PAK using GODOFWAR.TOC (24-byte records:
    /// name[16] + u32 offset + u32 size). Credits honest CD sectors for the member payload.
    /// </summary>
    private static bool TryHostLoadPakMember(Ps2System sys, string memberName, uint dest, uint maxBytes)
    {
        try
        {
            var mods = sys.IopModules;
            if (mods == null) return false;
            byte[]? toc = mods.ReadDiscFileBytes("GODOFWAR.TOC", 0x10000)
                          ?? mods.ReadDiscFileBytes("cdrom0:\\GODOFWAR.TOC", 0x10000);
            if (toc == null || toc.Length < 24) return false;

            string want = memberName.ToUpperInvariant();
            if (want.EndsWith(";1", StringComparison.Ordinal)) want = want[..^2];
            uint off = 0, size = 0;
            bool found = false;
            for (int i = 0; i + 24 <= toc.Length; i += 24)
            {
                int nameLen = 0;
                while (nameLen < 16 && toc[i + nameLen] != 0) nameLen++;
                if (nameLen == 0) continue;
                string n = System.Text.Encoding.ASCII.GetString(toc, i, nameLen).ToUpperInvariant();
                if (n != want) continue;
                off = (uint)(toc[i + 16] | (toc[i + 17] << 8) | (toc[i + 18] << 16) | (toc[i + 19] << 24));
                size = (uint)(toc[i + 20] | (toc[i + 21] << 8) | (toc[i + 22] << 16) | (toc[i + 23] << 24));
                found = true;
                break;
            }
            if (!found || size == 0) return false;
            if (size > maxBytes) size = maxBytes;
            // PART1.PAK is multi-GB — read only the member window via ISO LBA.
            // Root dir: PART1.PAK;1 at LBA 1547 (from ISO parse 2026-07-31).
            const uint PakLba = 1547;
            uint startLba = PakLba + (off / 2048u);
            uint skip = off % 2048u;
            uint needBytes = skip + size;
            uint nsec = (needBytes + 2047u) / 2048u;
            if (nsec == 0 || nsec > 512) nsec = Math.Min(nsec == 0 ? 1u : nsec, 512u);
            // Temporary buffer in high RDRAM (past dest) then copy.
            uint tmp = 0x01A00000u;
            if (tmp + nsec * 2048u >= (uint)SystemMemory.RDRAM_SIZE)
                tmp = 0x01800000u;
            uint got = sys.Cdvd.ReadSectorsTo(sys.Memory, startLba, nsec, tmp);
            if (got == 0) return false;
            uint destPhys = dest & 0x1FFFFFFFu;
            if (destPhys < 0x00100000u || destPhys + size >= (uint)SystemMemory.RDRAM_SIZE)
                return false;
            for (uint i = 0; i < size; i++)
                sys.Memory.Write8(destPhys + i, sys.Memory.Read8(tmp + skip + i));
            if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1")
                Console.Error.WriteLine(
                    $"[GOW] PART1 member \"{memberName}\" off=0x{off:X} size=0x{size:X} " +
                    $"-> 0x{destPhys:X8} sec={got} cdvd={sys.Cdvd.SectorsRead}");
            return true;
        }
        catch
        {
    ...
    private static bool WriteHostBytes(Ps2System sys, uint dest, byte[] data, uint maxBytes)
    {
        int n = data.Length;
        if (n > (int)maxBytes) n = (int)maxBytes;
        if (n <= 0) return false;
        uint destPhys = dest & 0x1FFFFFFFu;
        if (destPhys < 0x00100000u || destPhys + (uint)n >= (uint)SystemMemory.RDRAM_SIZE)
            return false;
        for (int i = 0; i < n; i++)
            sys.Memory.Write8(destPhys + (uint)i, data[i]);
        int sectors = (n + 2047) / 2048;
        if (sectors < 1) sectors = 1;
        sys.Cdvd.NoteHostReadSectors(sectors);
        return true;
    }

    /// <summary>Register a PART1.PAK TOC member as a virtual FILEIO stream (byte offset on disc).</summary>
    private static void TryOpenPakMemberFd(Ps2System sys, string memberName, uint pakBaseBytes)
    {
        var mods = sys.IopModules;
        if (mods == null) return;
        byte[]? toc = mods.ReadDiscFileBytes("GODOFWAR.TOC", 0x10000);
        ...
        for (int i = 0; i + 24 <= toc.Length; i += 24)
        {
            ...
            string n = System.Text.Encoding.ASCII.GetString(toc, i, nameLen).ToUpperInvariant();
            if (n != want) continue;
            uint off = (uint)(toc[i + 16] | (toc[i + 17] << 8) | (toc[i + 18] << 16) | (toc[i + 19] << 24));
            uint size = (uint)(toc[i + 20] | (toc[i + 21] << 8) | (toc[i + 22] << 16) | (toc[i + 23] << 24));
            if (size == 0 || size > 0x200000u) return;
            int fd = mods.FileOpenVirtualStream($"cdrom0:\\{memberName}", pakBaseBytes + off, size);
            if (Environment.GetEnvironmentVariable("DETPS2_TRACE_BIOS") == "1")
                Console.Error.WriteLine(
                    $"[GOW] virtual FILEIO \"{memberName}\" fd={fd} off=0x{off:X} size=0x{size:X}");
            return;
        }
    }
    ...
    private void TryArmPendingStreamJob(Ps2System sys, ulong c)
    {
        ...
        // Wave-9b/10/11B: never paint 989snd done-magic onto LoadWad streamObj / host shell
        // buffers / stream table slots (claim: pending=0x01CFE000 destroyed Fedo; claim100b
        // pending=0x2A1370 stomped table companion that holds streamObj pointer).
        bool refusePaint = pPhys is 0x01CFE000u or 0x01CFF000u
            || pPhys is >= 0x01CFD000u and < 0x01D00000u // DMA tag scratch + streamObj band
            || pPhys is >= 0x01D00000u and < 0x01F00000u // TIT1 / R_SHELL / TOC host windows
            || pPhys == 0x01E00000u
            // Stream table / budget / index block around 0x2A1358 (RefreshLoadWadStreamTable).
            || pPhys is >= 0x002A1300u and <= 0x002A1400u
            // Stream slot records base (poster gate *0x2A3310 + slots @0x2A3318).
            || pPhys is >= 0x002A3300u and <= 0x002A3800u
            || (_loadWadSeeded && pPhys == (sys.Memory.Read32(0x002A1358) & 0x1FFFFFFFu));
        // Also refuse if *pending already looks like streamObj magic/counter (0x100..)
        // or points at host R_SHELL — not a 989snd recv buffer.
        if (!refusePaint)
        {
            uint head = sys.Memory.Read32(pPhys);
            uint mid = sys.Memory.Read32(pPhys + 4);
            if (head is >= 0x00000100u and <= 0x00000200u && mid == 0x01E00000u)
                refusePaint = true;
            if (head == 0x4665646Fu) // Fedo
                refusePaint = true;
        }
        if (refusePaint)
        {
            sys.Memory.Write32(0x002A1338, 0);
            // Re-publish healthy streamObj if earlier stomps left 0xFFFFFFFF / 0.
            if (_loadWadSeeded)
            {
                uint so = sys.Memory.Read32(0x01CFE000u);
                if (so == 0xFFFFFFFFu || so == 0)
                    RefreshLoadWadStreamTable(sys);
            }
            return;
        }
