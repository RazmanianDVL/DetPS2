...
- **MK**: **NEAR-MENU / interactive-class** — gifP3=11 / dmac=**730** @120M pad-inject (was dmac=16); final PC in stream work `0x43FB60`; CROSS lifts gifP3 5→11; **"Kombat"/"Start"** in RDRAM. Wave-9: stream CAS re-arm (`*0x55E248`) + post-spine worker escape — stream body re-enters (dmac↑) but Path3 still plateaus 11; selection index unproven → **MENU = NEAR (not YES)**.
