...
| Mortal Kombat: Deception (USA) | `SLUS_208.81` | `MidwayFamilyAssist` **DEC** — idle force-process + gameart.ssf + PowerOff→keep-alive + **PL-012** pad inject + assist-stable sel-idx `*0x5DC000` + **PL-029** Host→Local SEC tex feed; no invent PATH3; Path3MaskedByVif held | **MENU YES + INTERACTIVE** p2qws=5988 px≈22M **imgBytes=557056** (G-GFX-3 partial); residual natural EE IMAGE + AnimMenu accept | 2026-07-31 |
